...
namespace serverMedIA
{
    public static class MainPublishersAnalysis
    {
        ...
        public static void SearchNews()
        {
            ...
            var termsToSearch = GetSearchQueries();

            if (termsToSearch != null)
            {
                foreach (var row in termsToSearch)
                {
                    idTerm = row.Item2;
                    term = row.Item3;
        ...
        {
            string datePublished = value["datePublished"] != null ? DateTime.Parse(value["datePublished"].ToString()).Subtract(new TimeSpan(5, 0, 0)).ToString("yyyy-MM-dd hh:mm:ss") : null;
        ...
        public static List<MultiLanguageInput> BuildOpinionRequest()
        {
            ...
            StringBuilder queryDescNews = new StringBuilder(@"SELECT TOP @MAX_NUMBER_OPINION id, description FROM News.News WHERE idOpinion IS NULL");
        ...
        public static List<MultiLanguageInput> BuildKeyPhraseRequest()
        {
            ...
            StringBuilder queryDescNews = new StringBuilder(@"SELECT TOP @MAX_NUMBER_KEY_PHRASE N.id, N.description FROM News.News AS N 
